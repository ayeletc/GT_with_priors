...
vecK                = [5]#[10]#[2, 3, 4, 5, 6]
nmc                 = 1000
enlarge_tests_num_by_factors = [0.4, 0.5, 0.6, 0.7, 0.8, 0.9, 1, 1.1, 1.2] #[0.25, 0.5, 0.75, 1, 1.25] #[0.5, 0.25, 0.5, 0.75, 1, 1.5]#[0.75, 0.8, 0.9, 1, 1.25, 1.5, 1.75, 2]# [0.5, 0.6, 0.7, 0.8, 0.9, 1, 1.3, 1.7, 2] #[0.85, 0.9, 0.95, 1, 1.25, 1.5, 1.75, 2]#
...
init_paths_number    = 500 # initial number of paths to find in list viterbi algorithm (in the 1st iiteration)
max_paths_for_lva    = 500 # TODO: try 40 and 60 (already done 80)
step_in_lva_paths    = 500
use_long_term_memory = True
viterbi_time_steps   = 1 #1/2/3
viterbi_comb_threshold = [30, 30, 20, 20, 20, 20, 20, 20, 20]#30 # 30 = K*10=> gamma = 10 = N/50 = N^0.03705 = 0.02*N; 
...
for method_DD in methods_DD:
        ...
        for idxT in range(num_of_test_scale):
            ...
            # for nn in range(nmc):
                ...
                if third_step_type == 'viterbi':
                    # # Opt 1
                    # path_trajs, path_probs, ml_prob, ml_traj = hmm_model.list_viterbi_algo_parallel_with_deter(observations, top_k=1)
                    # # remove dup rows
                    ...
                    while not possible_combination_found and top_k <= max_paths_for_lva:
                        if viterbi_time_steps == 1:
                            ...
                            # path_trajs, path_probs, ml_prob, ml_traj = hmm_model.list_viterbi_algo_parallel_with_long_memory(observations, top_k=top_k)
                            ...
                            if use_long_term_memory:
                                # TODO: if we change the name we need to update it in the dictionary!!
                        ...
                        # # remove paths that do no match the DD or PD prior
                        # defectives_in_paths = np.sum(paths, axis=1)
                        # relevant_paths = paths[np.where(np.bitwise_and(defectives_in_paths >= K,defectives_in_paths <=viterbi_comb_threshold))[0],:]

                        # paths = paths[:,]
                        # paths = paths[np.where(paths[:,DD2]==0)]
    ...
    if save_fig or save_raw:    
        ...
        experiment_str = 'N' + str(N) + '_nmc' + str(nmc) + '_methodDD_' + method_DD + permutations_label + '_thirdStep_' + third_step_label + viterbi_label + '_' + code_type + '_Tbaseline_' +  Tbaseline + '_'
